...
// drop retried webhooks. It's intentionally lightweight: Stripe retries
// for ~3 days with exponential backoff, so a few thousand entries in
// memory is enough to absorb realistic duplicate bursts. Persistence
// across restarts is unnecessary because the underlying CR patches are
// already idempotent — the deduper just saves a few wasted reconciles.
